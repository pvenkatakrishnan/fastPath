...
function pick(arr) {
    ...
    return ((entry, idx) => obj.hasOwnProperty(idx));
}
...
function filter(fn) {
    return function doFilter(arr) {
        ...
        Object.keys(arr).forEach(i => (fn(arr[i], i) !== undefined) && result.push(arr[i]));
        return result;
    };
}
...
function pickRange(start, end, step) {
    ...
    return (arr => (fn && fn(arr)) || arr.slice(start, end));
}
...
function mapAndFilter(fn) {
    return function doMapFilter(arr) {
        ...
        let current;
        Object.keys(arr).forEach(i => ((current = fn(arr[i], i)) !== undefined) && result.push(current));
        return result;
    };
}

function spread(fn) {
    return (arr => arr.reduce((result, current) => result.concat(fn(current) || []), []));
}
...
function handleArr({locator, processors, idx}, doSpread) {
    ...
    if(locator.indexOf('@') !== -1) {
        ...
        if(exp[2] === '*') {
            ...
            if(tmp.length < 3) {
                //n: or :n pattern
                processors.push(spread((idx === 0) ? pickRange(0, parseInt(tmp[1], 10)): pickRange(parseInt(tmp[0], 10))));
            } else {
                //start:end:step
                processors.push(spread(pickRange(parseInt(tmp[0], 10), parseInt(tmp[1], 10), parseInt(tmp[2], 10))));
            }
        } else {
            //[n], [1,2,...,n]
